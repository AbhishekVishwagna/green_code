...
var analyzeCode;
try {
  analyzeCode = require("./auditor-engine.cjs").analyzeCode;
} catch (e) {
  console.warn("[Green Code pod-server] auditor-engine.cjs failed to load:", e && e.message);
  analyzeCode = null;
}
var app     = express();
...
app.post("/audit", function(req, res) {
  ...
  if (!analyzeCode) return res.status(503).json({ error: "Auditor engine unavailable — auditor-engine.cjs failed to load" });
  try {
